...
int csp_rtable_set(uint8_t addr, uint8_t netmask, csp_iface_t *ifc,
                   uint16_t via) {
  (void)addr;
  (void)netmask;
  (void)ifc;
...
int csp_zmqhub_init(uint8_t addr, const char *server, uint16_t rx_filter,
                    csp_iface_t **iface) {
...
uint8_t csp_conn_src(csp_conn_t *conn) { return conn != NULL ? conn->src : 0; }

uint8_t csp_conn_dst(csp_conn_t *conn) { return conn != NULL ? conn->dst : 0; }
...
int csp_sfp_send_own_memcpy(csp_conn_t *conn, const void *data,
  ...
  memcpyfcn((csp_memptr_t)sfp_loopback.data, (csp_memptr_t)(uintptr_t)data,
            datasize);
