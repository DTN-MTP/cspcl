...
/* PHASE 1: Constants now centralized in cspcl_config.h
 ...
 * - CSP_ZMQHUB_ADDR = CSPCL_ZMQHUB_ADDR_DEFAULT
 * - CSP_CAN_IFACE = CSPCL_CAN_IFACE_DEFAULT
 * - CSP_IFACE_PARAM_MAX = CSPCL_IFACE_PARAM_MAX
 */

#define CSP_OBSERVE_MAX_NODE 32
#define CSP_DEFAULT_BP_PORT CSPCL_PORT_BP
#define CSP_RX_BUFFER_SIZE CSPCL_RX_BUFFER_SIZE
#define CSP_TX_BUFFER_SIZE CSPCL_TX_BUFFER_SIZE
#define CSP_ZMQHUB_ADDR CSPCL_ZMQHUB_ADDR_DEFAULT
#define CSP_CAN_IFACE CSPCL_CAN_IFACE_DEFAULT
#define CSP_IFACE_PARAM_MAX CSPCL_IFACE_PARAM_MAX
...
struct csp_cla_config {
  ...
  struct csp_peer peers[CSPCL_MAX_LINKS];
};

static enum csp_iface_type g_iface_type = CSP_IFACE_ZMQHUB;
static char g_zmqhub_addr[CSP_IFACE_PARAM_MAX] = CSP_ZMQHUB_ADDR;
static char g_can_iface[CSP_IFACE_PARAM_MAX] = CSP_CAN_IFACE;
...
{
  fprintf(stderr, "[cspcla] %s failed: %s (%d)\n", what, unibo_bp_error_to_string(err), (int) err);
  fflush(stderr);
}
...
static void log_info(const char *fmt, ...)
{
  ...
  va_list args;
  va_start(args, fmt);
  fprintf(stderr, "[cspcla %02d:%02d:%02d pid=%ld tid=%lu] ", tm_now.tm_hour, tm_now.tm_min,
          tm_now.tm_sec, (long) getpid(), (unsigned long) pthread_self());
  ...
  if (!initialized) {
    ...
    if (env && env[0] != '\0') {
      ...
      if (end != env && *end == '\0' && parsed > 0)
        max_bytes = (parsed > CSPCL_STREAM_CHUNK_SIZE) ? CSPCL_STREAM_CHUNK_SIZE : (size_t) parsed;
    }
    initialized = 1;
  }
...
static void log_payload_preview(const char *tag, const uint8_t *buffer, size_t len)
{
  ...
  log_info("%s payload preview (%zu/%zu bytes, rendered=%zu): %s%s", tag, preview_len, len,
           rendered_len, hexbuf, (rendered_len < len) ? " ..." : "");
}
...
/* PHASE 2: Address parsing now centralized in cspcl library
 * Use cspcl_parse_address() instead of parse_csp_addr()
 */
...
static int parse_iface_spec(const char *value, enum csp_iface_type *iface_type, char *zmqhub_addr,
                            size_t zmqhub_addr_len, char *can_iface, size_t can_iface_len)
{
...
static void print_usage(const char *prog)
{
  fprintf(stderr,
          ...
          "[bp_directory]\n"
          "Usage (flags):      %s -l <local_addr> -p <csp_port> [-i iface] [-c "
          "cla_id] [-d bp_directory]\n"
          "  iface: zmqhub | zmqhub:<host> | can | can:<iface> | loopback "
          "(default: zmqhub)\n"
          "         can uses vcan0 by default\n"
          "  bp_directory: Unibo-BP runtime directory (where unibo-bp was "
          "started)\n",
          prog, prog);
}
...
static int parse_daemon_args(int argc, char **argv, struct cspcl_daemon_config *cfg)
{
  ...
  cfg->bp_directory = ".";
  if (copy_iface_param(g_zmqhub_addr, sizeof(g_zmqhub_addr), CSP_ZMQHUB_ADDR) != 0)
    return -1;
  ...
  if (argc >= 3 && argv[1][0] != '-') {
    ...
    if (parse_uint8_arg(argv[2], &cfg->csp_port) != 0)
      return -1;
    has_csp_port = true;
    if (argc >= 4 && parse_iface_spec(argv[3], &cfg->iface_type, g_zmqhub_addr,
                                      sizeof(g_zmqhub_addr), g_can_iface, sizeof(g_can_iface)) != 0)
      return -1;
  ...
  while ((opt = getopt(argc, argv, "hl:p:i:c:d:")) != -1) {
    ...
    case 'p':
      ...
      break;
    case 'i':
      if (parse_iface_spec(optarg, &cfg->iface_type, g_zmqhub_addr, sizeof(g_zmqhub_addr),
                           g_can_iface, sizeof(g_can_iface)) != 0)
        return -1;
      break;
    case 'c':
...
{
  for (size_t i = 0; i < CSPCL_MAX_LINKS; ++i) {
...
static int ensure_link_for_addr(struct csp_cla_config *cfg, uint8_t csp_addr)
{
  ...
  size_t free_idx = CSPCL_MAX_LINKS;
  for (size_t i = 0; i < CSPCL_MAX_LINKS; ++i) {
    if (!cfg->peers[i].used) {
      free_idx = i;
      break;
  ...
  if (free_idx == CSPCL_MAX_LINKS) {
    log_info("no free peer slots available (max=%u)", CSPCL_MAX_LINKS);
    return -1;
  }
  ...
  uint64_t link_id = 0;
  err = unibo_bp_cla_create_link(cfg->handle, unibo_bp_const_cast_eid(ipn), true, true, &link_id);
  if (err != UniboBP_NoError) {
  ...
  err = unibo_bp_cla_set_link_param(cfg->handle, link_id, true, CSPCL_MAX_BUNDLE_SIZE);
  if (err != UniboBP_NoError) {
  ...
  err = unibo_bp_cla_start_observation_planned_neighbor(cfg->handle, unibo_bp_const_cast_eid(ipn));
  if (err != UniboBP_NoError) {
...
static void observe_default_peers(struct csp_cla_config *cfg)
{
  ...
  for (uint8_t node = 1; node <= CSP_OBSERVE_MAX_NODE; ++node) {
    ...
    err =
        unibo_bp_cla_start_observation_planned_neighbor(cfg->handle, unibo_bp_const_cast_eid(ipn));
    if (err != UniboBP_NoError)
      log_bp_error("unibo_bp_cla_start_observation_planned_neighbor", err);
    else
      observed++;
...
static bool on_outbound_pdu(uint64_t link_id, ConstUniboBPCLAOutboundPDU pdu, void *user_arg)
{
  ...
  log_info("outbound_pdu callback link_id=%" PRIu64 " tx_id=%" PRIu64 " len=%" PRIu64, link_id,
           tx_id, len);
  ...
  while (remaining > 0) {
    const size_t chunk =
        (remaining > CSPCL_STREAM_CHUNK_SIZE) ? CSPCL_STREAM_CHUNK_SIZE : (size_t) remaining;
  ...
  cspcl_error_t cerr =
      cspcl_send_bundle(&cfg->cspcl, buffer, (size_t) len, peer->csp_addr, cfg->csp_port);
  free(buffer);

  if (cerr == CSPCL_OK) {
    log_info("✓ TX SUCCESS tx_id=%" PRIu64 " dst_csp=%u:%u bytes=%" PRIu64 " (all ACKs received)",
             tx_id, peer->csp_addr, cfg->csp_port, len);
  ...
  if (cerr == CSPCL_ERR_TIMEOUT) {
    log_info("✗ TX FAILURE (TIMEOUT) tx_id=%" PRIu64 " dst_csp=%u:%u - receiver offline "
             "or ACKs lost (link quality issue)",
             tx_id, peer->csp_addr, cfg->csp_port);
  } else if (cerr == CSPCL_ERR_CONNECTION) {
    log_info("✗ TX FAILURE (CONNECTION) tx_id=%" PRIu64 " dst_csp=%u:%u - link broken", tx_id,
             peer->csp_addr, cfg->csp_port);
  } else if (cerr == CSPCL_ERR_BUNDLE_TOO_LARGE) {
    log_info("✗ TX FAILURE (TOO LARGE) tx_id=%" PRIu64 " bytes=%" PRIu64 " exceeds maximum", tx_id,
             len);
  } else {
    log_info("✗ TX FAILURE tx_id=%" PRIu64 " dst_csp=%u:%u cspcl_err=%d (%s)", tx_id,
             peer->csp_addr, cfg->csp_port, (int) cerr, cspcl_strerror(cerr));
  }
...
{
  (void) user_arg;
  log_info("update_config_flags callback: enable=0x%" PRIx64 " disable=0x%" PRIx64, enabled_flags,
           disabled_flags);
}

static void on_add_contact(ConstUniboBPEID neighbor, bool uplink, struct timeval *start_time,
                           struct timeval *end_time, uint64_t xmit_rate_Bps, void *user_arg)
{
  (void) start_time;
  (void) end_time;
  (void) xmit_rate_Bps;
  ...
  if (ensure_link_for_addr(cfg, csp_addr) == 0) {
    ...
    if (peer) {
      ...
      if (err != UniboBP_NoError)
        ...
        log_info("add_contact callback: opened link_id=%" PRIu64 " for csp_addr=%u", peer->link_id,
                 csp_addr);
    }
  }
  pthread_mutex_unlock(&cfg->lock);
...
static void on_remove_contact(ConstUniboBPEID neighbor, bool uplink, struct timeval *start_time,
                              void *user_arg)
{
  struct csp_cla_config *cfg = user_arg;
  const uint8_t csp_addr = csp_addr_from_eid(neighbor);
...
static void on_add_range(ConstUniboBPEID neighbor, bool uplink, struct timeval *start_time,
                         struct timeval *end_time, uint64_t owlt_s, void *user_arg)
{
  const uint8_t csp_addr = csp_addr_from_eid(neighbor);
  log_info("add_range callback: uplink=%d csp_addr=%u owlt=%" PRIu64, (int) uplink, csp_addr,
           owlt_s);
  (void) start_time;
  (void) end_time;
  (void) user_arg;
...
static void on_remove_range(ConstUniboBPEID neighbor, bool uplink, struct timeval *start_time,
                            void *user_arg)
{
  const uint8_t csp_addr = csp_addr_from_eid(neighbor);
...
static void *csp_rx_task(void *arg)
{
  ...
  while (cfg->rx_running) {
    ...
    cspcl_error_t err =
        cspcl_recv_bundle(&cfg->cspcl, bundle_buffer, &bundle_len, &src_addr, &src_port, 1000);
    ...
    if (err != CSPCL_OK) {
      ...
      if (err == CSPCL_ERR_SFP) {
        log_info("✗ RX ERROR (SFP REASSEMBLY) src_csp=%u:%u - incomplete bundle received "
                 "(fragments missing, discarding)",
                 src_addr, src_port);
      } else if (err == CSPCL_ERR_CSP_RECV) {
        log_info("✗ RX ERROR (CSP_RECV) src_csp=%u:%u - link issue", src_addr, src_port);
      } else {
        log_info("✗ RX ERROR src_csp=%u:%u cspcl_err=%d (%s)", src_addr, src_port, (int) err,
                 cspcl_strerror(err));
      }
      continue;
    }

    log_info("✓ RX COMPLETE src_csp=%u:%u bytes=%zu (all fragments received)", src_addr, src_port,
             bundle_len);
    ...
    while (remaining > 0) {
      size_t chunk = (remaining > CSPCL_STREAM_CHUNK_SIZE) ? CSPCL_STREAM_CHUNK_SIZE : remaining;
...
static int csp_cla_init(struct csp_cla_config *cfg, uint8_t local_addr, uint8_t csp_port,
                        uint64_t cla_id, const char *bp_directory)
{
  ...
  log_info("csp_cla_init begin local_addr=%u csp_port=%u cla_id=%" PRIu64 " bp_dir=%s", local_addr,
           csp_port, cla_id, bp_directory ? bp_directory : "(null)");
  ...
  strncpy(cfg->cspcl.zmqhub_addr, g_zmqhub_addr, CSP_IFACE_PARAM_MAX - 1);
  cfg->cspcl.zmqhub_addr[CSP_IFACE_PARAM_MAX - 1] = '\0';
  ...
  log_info("trace settings: CSPCL_TRACE=%s CSPCLA_TRACE_PAYLOAD=%s "
           "CSPCLA_TRACE_BYTES=%s",
...
static void csp_cla_cleanup(struct csp_cla_config *cfg)
{
  ...
  for (size_t i = 0; i < CSPCL_MAX_LINKS; ++i) {
    ...
    if (cfg->peers[i].eid)
      (void) unibo_bp_cla_stop_observation_planned_neighbor(
          cfg->handle, unibo_bp_const_cast_eid(cfg->peers[i].eid));
...
int csp_cla_create_unibo_update(const char *const options[], size_t option_count)
{
  ...
  if (option_count >= 3) {
    if (parse_iface_spec(options[2], &g_iface_type, g_zmqhub_addr, sizeof(g_zmqhub_addr),
                         g_can_iface, sizeof(g_can_iface)) != 0)
      return -1;
  }
  ...
  log_info("host-managed create successful local_addr=%u csp_port=%u "
           "cla_id=%" PRIu64,
...
int main(int argc, char **argv)
{
  ...
  if (csp_cla_init(&cfg, daemon_cfg.local_addr, daemon_cfg.csp_port, daemon_cfg.cla_id,
  ...
  fprintf(stdout,
          ...
          daemon_cfg.local_addr, daemon_cfg.csp_port, (int) daemon_cfg.iface_type,
          daemon_cfg.cla_id, daemon_cfg.bp_directory);

  while (!g_shutdown_requested && cfg.rx_running)
    sleep(1);

  log_info("main loop exiting shutdown_requested=%d rx_running=%d", (int) g_shutdown_requested,
           (int) cfg.rx_running);
  csp_cla_cleanup(&cfg);
  g_active_cfg = NULL;
  return EXIT_SUCCESS;
