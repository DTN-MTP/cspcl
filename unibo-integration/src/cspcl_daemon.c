...
static void log_info(const char *fmt, ...)
{
  ...
  va_list args;
  va_start(args, fmt);
  fprintf(stderr, "[cspcla %02d:%02d:%02d pid=%ld tid=%lu] ", tm_now.tm_hour,
          tm_now.tm_min, tm_now.tm_sec, (long)getpid(),
                                ...
                                size_t len)
{
  ...
  log_info("%s payload preview (%zu/%zu bytes, rendered=%zu): %s%s", tag,
           preview_len, len, rendered_len, hexbuf,
           (rendered_len < len) ? " ..." : "");
}
                             ...
                             struct cspcl_daemon_config *cfg)
{
  ...
  cfg->bp_directory = ".";
  if (copy_iface_param(g_zmqhub_addr, sizeof(g_zmqhub_addr), CSP_ZMQHUB_ADDR) !=
      0)
    return -1;
  ...
  while ((opt = getopt(argc, argv, "hl:p:i:c:d:")) != -1)
  {
    ...
    case 'p':
      ...
      break;
    case 'i':
      if (parse_iface_spec(optarg, &cfg->iface_type, g_zmqhub_addr,
                           sizeof(g_zmqhub_addr), g_can_iface,
                           sizeof(g_can_iface)) != 0)
        return -1;
      break;
...
static uint8_t csp_addr_from_eid(ConstUniboBPEID eid)
{
  ...
  if (unibo_bp_eid_get_scheme(eid) == UniboBPScheme_ipn)
    return (uint8_t)unibo_bp_eid_ipn_get_node_number(
        unibo_bp_const_cast_eid_ipn(eid));
...
static int ensure_link_for_addr(struct csp_cla_config *cfg, uint8_t csp_addr)
{
  ...
  log_info("link ready csp_addr=%u link_id=%" PRIu64 " slot=%zu", csp_addr,
           link_id, free_idx);
                            ...
                            void *user_arg)
{
  ...
  log_info("outbound_pdu callback link_id=%" PRIu64 " tx_id=%" PRIu64
           " len=%" PRIu64,
           link_id, tx_id, len);
                           ...
                           uint64_t xmit_rate_Bps, void *user_arg)
{
  ...
  log_info("add_contact callback: uplink=%d csp_addr=%u", (int)uplink,
           csp_addr);
  ...
  if (ensure_link_for_addr(cfg, csp_addr) == 0)
  {
    ...
    if (peer)
    {
      ...
      if (err != UniboBP_NoError)
        ...
        log_info("add_contact callback: opened link_id=%" PRIu64
                 " for csp_addr=%u",
  ...
  if (cfg)
  {
    ...
    if (peer)
    {
      ...
      if (err != UniboBP_NoError)
        ...
        log_info("remove_contact callback: closed link_id=%" PRIu64
                 " for csp_addr=%u",
                 peer->link_id, csp_addr);
    }
    pthread_mutex_unlock(&cfg->lock);
...
static void *csp_rx_task(void *arg)
{
  ...
  while (cfg->rx_running)
  {
    ...
    cspcl_error_t err = cspcl_recv_bundle(
        &cfg->cspcl, bundle_buffer, &bundle_len, &src_addr, &src_port, 1000);
                        ...
                        const char *bp_directory)
{
  ...
  log_info("csp_cla_init begin local_addr=%u csp_port=%u cla_id=%" PRIu64
           " bp_dir=%s",
  ...
  unibo_bp_cla_register_callback_update_config_flags(cfg->handle,
                                                     on_update_config_flags);
  ...
  log_info("trace settings: CSPCL_TRACE=%s CSPCLA_TRACE_PAYLOAD=%s "
           ...
           getenv("CSPCLA_TRACE_PAYLOAD") ? getenv("CSPCLA_TRACE_PAYLOAD")
                                          : "(unset)",
           getenv("CSPCLA_TRACE_BYTES") ? getenv("CSPCLA_TRACE_BYTES")
                                        : "(unset)");
...
int main(int argc, char **argv)
{
  ...
  fprintf(stdout,
          "[cspcla] running local_addr=%u csp_port=%u iface=%d cla_id=%" PRIu64
          " bp_dir=%s\n",
          daemon_cfg.local_addr, daemon_cfg.csp_port,
          (int)daemon_cfg.iface_type, daemon_cfg.cla_id,
          daemon_cfg.bp_directory);

  while (!g_shutdown_requested && cfg.rx_running)
    sleep(1);

  log_info("main loop exiting shutdown_requested=%d rx_running=%d",
           (int)g_shutdown_requested, (int)cfg.rx_running);
  csp_cla_cleanup(&cfg);
  g_active_cfg = NULL;
  return EXIT_SUCCESS;
}
