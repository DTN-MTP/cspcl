    ...
    if (!cspcl->initialized) {
        ...
        if (!g_cspcl_stack_initialized) {
            ...
            case CSP_IFACE_ZMQHUB:
                ret =
                    csp_zmqhub_init(cspcl->local_addr, cspcl->zmqhub_addr, 0, &cspcl->active_iface);
                if (ret != CSP_ERR_NONE) {
#ifndef FREERTOS
                    pthread_mutex_unlock(&g_cspcl_global_lock);
#endif
                                ...
                                uint8_t dest_addr, uint8_t dest_port)
{
    ...
    int ret =
        csp_sfp_send(conn, bundle, (unsigned int) len, CSPCL_MAX_PAYLOAD, CSPCL_CSP_TIMEOUT_MS);
